...
// Hash the DECODED pixels (not the PNG file bytes). Two images with identical
// pixels but different PNG encodings (e.g. a Photoshop export vs. this plugin's
// pngjs output) share this key — file-byte MD5 does NOT, which is why the old
// whole-assets dedup missed images the project already had. Dimensions are mixed
// in so different-sized images never collide.
function pixelKey(rgba, w, h) {
    if (!rgba || !w || !h) return null;
    const hsh = crypto.createHash('md5');
    hsh.update(w + 'x' + h + ':');
    hsh.update(Buffer.isBuffer(rgba) ? rgba : Buffer.from(rgba.buffer, rgba.byteOffset, rgba.byteLength));
    return hsh.digest('hex');
}

// Build a project-wide dedup table by scanning EVERY sprite-frame PNG under
// assets/ (not just this conversion's outDir). Keyed by the PNG's DECODED-PIXEL
// hash so it matches images the project already has regardless of who encoded
// them; the value reuses the asset's existing sprite-frame uuid (from its
// .png.meta). Read-only: never written back to .psd_assets.json.
function buildGlobalDedupTable(assetsRoot) {
    ...
    while (stack.length) {
        ...
        for (const ent of ents) {
            ...
            if (!sf) continue;
            let dec; try { dec = PNG.sync.read(fs.readFileSync(full)); } catch (e) { continue; } // decode to RGBA
            const key = pixelKey(dec.data, dec.width, dec.height);
            if (!key || tbl[key]) continue; // first match wins (stable)
            const rel = path.relative(assetsRoot, full).split(path.sep).join('/');
            tbl[key] = { png: rel, spriteUuid: sf.uuid, w: dec.width, h: dec.height };
        }
    }
    return tbl;
}
...
async function convert(opts) {
    ...
    function resolveImage(node) {
        ...
        // Project-wide reuse: an image with the SAME PIXELS already exists somewhere
        // in assets (matched by decoded-pixel hash, not file bytes) — point at its
        // existing sprite-frame instead of writing a new image.
        if (!res && globalTable && eligible) {
            const gk = pixelKey(png.rgba, png.width, png.height);
            const ent = gk ? globalTable[gk] : null;
            if (ent) { report.reused++; res = { spriteUuid: ent.spriteUuid, w: png.width, h: png.height }; }
        }
        if (!res) {
